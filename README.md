DigitalFreq35
Copyright (c) 2026 Berke

An open-source desktop music player application.

License
-------
This project is licensed under the GNU General Public License v3.0 or later (GPLv3+).
See the LICENSE.txt file for details.

Third-Party Components
----------------------
This application uses the following open-source components:

- PySide6 / Qt (LGPLv3)
- NumPy
- sounddevice
- soundfile
- Python
- Share Tech Mono font (OFL)
- Noto Sans Mono font (OFL)

Their respective license texts are included in the
third_party_licenses directory.

PySide6 (Qt) is dynamically linked in accordance with LGPL requirements.
Users may replace or modify the Qt libraries under the terms of the LGPL.
